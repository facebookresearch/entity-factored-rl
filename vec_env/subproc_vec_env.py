...
# Based on the vec_env implementations at https://github.com/DLR-RM/stable-baselines3.
...
def _worker(remote, parent_remote, env_fn_wrapper, auto_reset):
    parent_remote.close()
    with mujoco_py.ignore_mujoco_warnings():
        env = env_fn_wrapper.var()
        # Ignore "Nan, Inf or huge value in QACC" issues
        while True:
            try:
...
class SubprocVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, start_method=None, auto_reset=False):
        self.waiting = False
        self.closed = False
        n_envs = len(env_fns)
        ...
        for work_remote, remote, env_fn in zip(self.work_remotes, self.remotes, env_fns):
            args = (work_remote, remote, CloudpickleWrapper(env_fn), auto_reset)
